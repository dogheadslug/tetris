...
int __stdcall WinMain(HINSTANCE hInstance, HINSTANCE hPreInstance, LPTSTR LPCmdLine, int nCmdShow) {
	...
	wc.hCursor = LoadCursor(hInstance, MAKEINTRESOURCE(IDC_POINTER)); //type of cursor
	...
	//creating a window
	hWnd = CreateWindowEx(WS_EX_TOPMOST, "tetris", "test", WS_OVERLAPPEDWINDOW, 100, 100, 500, 650, NULL, NULL, hInstance, NULL);
	if (NULL == hWnd) {
		return 0;
	}
